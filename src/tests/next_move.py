...
positions = [
    ('r1bq1r1k/p1pnbpp1/1p2p3/6p1/3PB3/5N2/PPPQ1PPP/2KR3R w - -', 'g4')
]
...
class NextMoveTest(unittest.TestCase):
    ...
    def test_positions(self):
        for p in positions:
            (position, best_move) = p
            with self.subTest(position=position, best_move=best_move):
                transposition_table = TranspositionTable()
                board = Board(position)
                move = Searcher(board, 3, transposition_table).next_move()
                self.assertEqual(str(move), best_move)
